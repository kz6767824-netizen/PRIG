...
def _build_prompt(
    ...
    return f"""You are writing one section of a schema-change impact report for a data engineering team. Be concrete and concise (roughly 3-5 sentences).
...
Governance flags: {governance_flags or 'none'}
...
def llm_explanation(
    ...
    if not api_key:
        ...
        return template_explanation(
            table, [operation], severity, downstream_assets, is_pii, governance_flags, new_column
        )
    ...
    try:
        ...
        # FIX: Bullet-point header so it renders on separate lines
        header = (
            f"- **Table:** `{table}`\n"
